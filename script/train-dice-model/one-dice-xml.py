...
TRIAL = 6
NUM_CLASSES = 6
IGNORED_CLASSES = ['dados']
USE_MIXUP = True
USE_CUTMIX = False
ALPHA = 0.2

RAW_DATASET_PATH = 'train/image/random-dices-xml'
PARSED_DATA_PATH = 'train/image/parsed-dices-xml'
MODEL_SAVE_PATH = f'model/dice-model/dices-xml'
RAPORT_SAVE_PATH = f'raport/train-dice-model/dices-xml'
...
class_names = sorted([d for d in os.listdir(PARSED_DATA_PATH) if os.path.isdir(os.path.join(PARSED_DATA_PATH, d))])
class_to_idx = {cls: idx for idx, cls in enumerate(class_names)}
image_paths = []
labels = []

for cls in class_names:
    for path in glob(os.path.join(PARSED_DATA_PATH, cls, '*.jpg')):
